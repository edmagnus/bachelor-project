...
# ADDED FOR GHD ----------------------------------------
def ghd_char_fct_vec(z,lam,alpha,beta,delta,mu,Lambda,device):
    """ z is a tensor of shape (n,d) and corresponds arguments of characteristic function of d-dimensional Generalized Hyperbolic distribution
        lam is the lambda parameter
        alpha is the alpha parameter
        beta is a tensor or array of shape (d,) and corresponds to the skewness vector
        delta is the delta parameter
        mu is a tensor or array of shape (d,) and corresponds to the location vector
        Lambda is a tensor or array of shape (d,d) and corresponds to the scale matrix
    """
    if z.dim()==1:
        z=z.reshape(1,z.shape[0])

    z_np=z.detach().cpu().numpy().astype(np.float64)
    beta_np=np.asarray(beta.detach().cpu().numpy() if torch.is_tensor(beta) else beta,dtype=np.float64)
    mu_np=np.asarray(mu.detach().cpu().numpy() if torch.is_tensor(mu) else mu,dtype=np.float64)
    Lambda_np=np.asarray(Lambda.detach().cpu().numpy() if torch.is_tensor(Lambda) else Lambda,dtype=np.float64)

    if beta_np.ndim!=1:
        beta_np=beta_np.reshape(-1)
    if mu_np.ndim!=1:
        mu_np=mu_np.reshape(-1)

    d=z_np.shape[1]
    if beta_np.shape[0]!=d or mu_np.shape[0]!=d or Lambda_np.shape!=(d,d):
        print("Dimensions inconsistent" ,[z.shape, beta_np.shape, Lambda_np.shape, mu_np.shape])
        return cmath.nan

    gamma2=alpha**2-beta_np @ Lambda_np @ beta_np
    if gamma2<=0:
        raise ValueError("Invalid GHD parameters: need alpha^2 > beta^T Lambda beta.")
    if delta<=0:
        raise ValueError("Invalid GHD parameters: this implementation assumes delta > 0.")

    gamma=np.sqrt(gamma2)
    denominator=scipy.special.kv(lam,delta*gamma)
    if denominator==0 or not np.isfinite(denominator):
        raise FloatingPointError("Bessel denominator K_lambda(delta*gamma) is zero or non-finite.")

    complex_shift=beta_np[None,:]+1j*z_np
    U=np.einsum('ni,ij,nj->n',complex_shift,Lambda_np,complex_shift)
    q=alpha**2-U
    values=np.exp(1j*(z_np @ mu_np))*np.power(gamma2/q,lam/2)*scipy.special.kv(lam,delta*np.sqrt(q))/denominator

    return torch.tensor(values,dtype=torch.complex64).to(device)


def ghd_char_fct(z,lam,alpha,beta,delta,mu,Lambda,device):
    """ z is a tensor of shape (d,) and corresponds one argument of the characteristic function of d-dimensional Generalized Hyperbolic distribution
        This is a scalar wrapper around ghd_char_fct_vec.
    """
    return ghd_char_fct_vec(z.reshape(1,z.shape[0]),lam,alpha,beta,delta,mu,Lambda,device)[0]
